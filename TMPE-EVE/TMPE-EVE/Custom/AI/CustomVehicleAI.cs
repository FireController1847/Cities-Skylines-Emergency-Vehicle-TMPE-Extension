...
using EmergencyVehicleExtension.Manager.Impl;
using System;
...
namespace EmergencyVehicleExtension.Custom.AI {
    class CustomVehicleAI : VehicleAI {
        public void CustomUpdatePathTargetPositions(ushort vehicleID, ref Vehicle vehicleData, Vector3 refPos, ref int targetPosIndex, int maxTargetPosIndex, float minSqrDistanceA, float minSqrDistanceB) {
            // Log._Debug($"EmergencyVehicleExtension.CustomVehicleAI.CustomUpdatePathTargetPositions({vehicleID}): Called!");
            ...
            while (true) {
                ...
                if ((vehicleData.m_flags & Vehicle.Flags.Emergency2) != (Vehicle.Flags)0) {
                    if (!Options.buildingOverlay) {
                        ...
                        bestLaneIndex = EmergencyVehicleManager.Instance.FindBestLane(vehicleID, ref vehicleData, nextPathPos);
                    }
                } else {
